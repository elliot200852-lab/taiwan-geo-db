...
- 2026-09-08：**批 6 鹽水上線＝15/37**（頁＋hero 同一 commit；4,860 字／教學 36.0%／圖 10／源 35；`fact_gate --fetch` 79 條全部指得到、**引文命中 79、「?」0 條**——臺南批第一頁零二進位缺口；查核 Sonnet **高 0 中 0 低 2**〔低 1 補來源、低 2 是「頁面未 build」流程狀態〕；fresh 驗收 GO；build 後 `<blockquote>` 內 `<li>` 9 個＝3 段×3 欄；`--check-images` 臺南 15 頁全過、零 429）。查核＝`_workspace/geo-tn-b6/yanshui-check.md`、驗收＝`yanshui-acceptance.md`。
