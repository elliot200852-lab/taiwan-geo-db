...
## 0. 下一個 session 從這裡開始（2026-07-28 深夜收官更新）

**一句話**：新北 22/29 已上線——第三批 6 區（永和、蘆洲、五股、泰山、樹林、土城＝
淡水河左岸／大漢溪走廊）於 2026-07-28 深夜收官（commit bfa142f，CI 綠、
`verify_live_images.py` 641/641 全綠）。查核 A/B 兩組修 6 高 24 中；檢索 golden
五組內容位移更新（媽祖/颱風/九份/濕地/日治時期，引擎零改動——內容進榜就更新 golden
並在 commit 註明，這是第三次沿用的前例）。

### 欠帳（兩件，David 拍板才動）
1. **第三批 6 頁 hero 未生**（`hero: false` 掛著）——生圖需 David 當次同意（生完記得拿旗標）。
2. **土城說書稿#1**：查核發現「隨車員工作日誌」與新店「工地日誌」同族（文件體日誌），
   備選方案「行車檢查簿」已擬好，等 David 一句話（不換也能活，批次感輕微）。

### 下一批（等 David 指示才動工）
剩 7 區：深坑、石碇、坪林、雙溪（山區）＋三芝、石門、萬里（北海岸）——正好是
一批完整的收官批。動工時照 §4 續做 SOP（含紅隊關卡）＋§7 派工模板；查核照本輪模式
（寫作與查核分開派、修正另派、一批收齊才 commit）。第三批的紀律補充：這批山區／
海岸區地形性強，回到第一批「一區一地形」模板即可，不需要第三批的都會區逐區破題法；
看門狗改餵 agent 自寫的 heartbeat 檔（transcript stub 會誤報），heartbeat 至少每
10 分鐘一行＋閾值 20 分鐘實測可用。

### 本輪留下的紀律（新進的必讀）
- 派工單必含**外部站活性紀律**與 **TCMB 線上站禁令**（§6b 凍死驗屍＋模板 §F/§G）。
- 背景 agent 派出即掛看門狗；卡死先驗屍再定死因；救援＝TaskStop→SendMessage（§6b）。
