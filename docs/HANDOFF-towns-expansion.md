...
- 2026-09-07：**批 5 後半 七股——初稿與機械閘完成，未上線**（`content/tainan/qigu.md` 在磁碟；towns_status 讀 4,922 字／教學 36.5%／圖 10／源 27／速覽 123；`fact_gate --fetch` **82/82 全命中、「?」0 條**，factmap＝`_workspace/geo-tn-b5/qigu-factmap.txt`）。**尚未做：查核（Sonnet 1 支）→ 修正 → fresh 驗收 → 圖片鏈／hero → build／發布／verify。**
  - **主對話待改兩處**（§0-TN 步驟 5，不派 agent）：①〈定位速覽〉AI 腔 1 條「不是等高線，是海堤、鹽田的埕堤與魚塭的土堤。」（禁用句式）；②字數 4,922 超上限 22 字，順手一起收。寫作端已按住不動正文，改動後 factmap 編號會變、要重跑 fact_gate。
...
- 下一批＝**批 5 後半 七股（1 區，TN-S1 收官區）**：七股在西港（批 3）之後，**七股頁要對照北門定稿的停曬三年份**（plan §4-B）＋七股潟湖屬臺江內海遺跡不屬倒風內海（§2-A）。派工單＝沿用 `b5/tn-b5-common.md`（已是單支序做版）＋新寫 `tn-b5-qigu.md`（主軸 plan §2-B 第 21 列＝「停曬之後這片地被誰用了」）；查核單 `tn-b5-check.md` M 項已含七股條。七股上線後＝TN-S1 12 區收官，照 §0-KH 第 8 點寫 session handoff、記額度。
