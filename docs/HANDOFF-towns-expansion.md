...
- **全站互指總掃描**：38 區寫完後應做一次；本 session 已確認前批田寮／小港指向岡山、林園、鳥松、杉林的四處前向指涉現在都指得到。
- **全站 placenames 簡稱統一**：本批 8 頁已統一為〈地名裡的臺灣〉，其餘已上線頁仍有變體。
- **worktree／分支清理**：`_workspace/geo-kh-wt` 與 `kh-towns` 分支在高雄批結案後可移除。
- **NotebookLM 補檔**：「認識臺灣」筆記本目前 94 來源，花蓮 13＋臺東 16＋高雄 38 尚未補（**要先問 David**）。
