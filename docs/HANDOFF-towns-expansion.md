...
**2026-08-09 收單（David 拍板：批二後收線，KH-S1 由兩個 session 完成——切點以他當日指示為準）**

- **6/38 母本定稿、逐頁獨立過閘、已 commit 推 origin/kh-towns**：
  批一 90d5ac3（鼓山、橋頭、那瑪夏）＋批二（鹽埕、燕巢、梓官）。
  六頁腳本口徑 5,180–5,198 字全壓上限內、教學 35.8–39.9%、速覽 115–132、
  圖 51 張全 Commons 三欄齊零壞圖、AI 腔零。**未查核、未圖鏈、未 hero、未發布**——
  KH-S1 的查核 A/B 在寫滿 12 區後才跑（固定流程步驟 5 起）。
- **敏感頁已過的**：那瑪夏（A 級：零傷亡數字、全篇標時點）、鹽埕（B 級二二八：
  空間事實、機關名表述）。
- **下一 session 接手序列**：①開工照固定流程 1–3（preflight → worktree 內
  `towns_status.py kaohsiung` 應報 6/38 → git fetch；worktree 不在就從 origin/kh-towns 重開，
  記得重建 `_workspace/taiwan-arts-db` symlink）②紅隊關卡 §5 重跑 ③used-images 模板 §G
  程式碼在 worktree 重生 ④**發批三：前鎮、大樹、茂林**（§8-3 三格照抄；派工單樣式照批一批二——
  模板全文＋§D 該區行＋§8-3 三格＋已完成鄰區與《說書稿切分提示》避近似＋心跳＋
  towns_status 自檢口徑）→ 批四：新興、彌陀、六龜（**六龜 A 級**：派工單附
  `content/themes/typhoon.md` 莫拉克段＋那瑪夏頁當口徑對照）⑤12 區收齊 → 查核 A＋
  查核 B×2 → 修正 → 圖鏈 → hero 12 張 → 發布 → 更新本節（固定流程 5–8）。
- **隱性狀態清單**：worktree `_workspace/geo-kh-wt`@kh-towns（已推 origin，與本地一致）；
  `_workspace/taiwan-arts-db` symlink 已建；`docs/devices-used-kaohsiung.txt` 已收批一批二
  18 裝置全文；`hero-prompts.yaml` 高雄 0 條（未動）；used-images 清單在 scratchpad 會被系統清，
  用模板 §G 重生；看門狗未掛（本 session 用 `agent-liveness-watch.sh 20 300 150 <transcript 真檔>`，
  ⚠ tasks/*.output 是 symlink，要先 `readlink -f` 再餵）。
- **跨頁備忘（寫作 agent 查證出、後批必用）**：
  台積電高雄廠在**楠梓**（南科楠梓園區），橋頭是另一個南科橋頭園區——批五楠梓頁用；
  捷運紅線北端終點南岡山在**岡山區**——批十二岡山頁用；
  壽山本身也被淺野水泥採過石灰岩（1917–1992 停採），鼓山頁已寫——楠梓寫半屏山時互指即可；
  蚵仔寮作息＝凌晨出海**中午拍賣**（不是清晨卸魚）；
  ⚠ **高雄里編組 2026 連環調整**（那瑪夏 115-07 起 4 里、鹽埕 115-07 起 20 里，
  民政局 115-03 表仍舊制）——引表1 時里鄰數與現制可能不一致，照鹽埕做法「採現制並標時點」；
  民政局最新可得月＝115-03（發佈 115-04-02），下一 session 先查有無更新月。
- **校準結論（批一批二＝小樣）**：Commons 圖量能無虞（六頁 8–10 張全過，含市中心小區鹽埕）；
  民政局表1 xlsx 鏈穩定；**面積官方線上取不到**（主計處無區級表、多個區公所頁無數字）→
  沿用「維基彙整＋如實標層級」，全批收官時列欠帳問 David 要不要補紙本年報。
- 本 session 紅隊 Stage 1 全文（26 條）在 session 紀錄；裁決摘要見上節。
